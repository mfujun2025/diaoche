...
> **当前状态：全部完成 ✅（方案已从 Access 改为应用层密钥，2026-09-21 线上验证通过）**
...
| **后台密钥登录（`DC_ADMIN_KEY`）** | ✅ **代码完成 + 线上实测通过** |
| **删除 Access 应用** | ✅ **已完成（2026-09-21 上午）** |
| **控制台加 Secret `DC_ADMIN_KEY`** | ✅ **已完成**（注意：有 1 条重复待清理） |
| **车源详情页 `/trucks/<id>/`** | ✅ **已完成 + 线上实测通过**（按需渲染，带 SEO 信息） |
| **联系方式接口 + 登录门禁** | ⚠️ **已接好管线，登录为占位**（后期接短信/微信，只改两处） |
| **线上事后验证** | ✅ `/admin/`=200、无密钥 401、对密钥 200、错密钥 401、前台 200 |
| ~~Access 保护后台~~ | ⚠️ **已废弃**（Pages 自定义域名冲突，见第二节） |
| ~~`ADMIN_EMAILS` 白名单~~ | ⚠️ **已废弃**（代码不再读取） |
| 清理重复的 `DC_ADMIN_KEY` Secret | ⬜ 控制台里有两条同名，删掉一条 |
| **接真实用户登录**（看联系方式） | ⬜ 需先定短信服务或微信开放平台；代码只改 `requireLogin()` 和 `bindLoginGate()` 两处 |
| 上传接口鉴权 | ⬜ 待你定策略（可选 Turnstile） |
| 清理线上测试数据 | ⬜ 3 条 pending 测试车源（#2 #3 #4，前台不可见，无影响） |
| 删除临时 CF token | ⬜ `cfut_Urh2...`、`cfut_zRdge...`（都已失效/用完） |
